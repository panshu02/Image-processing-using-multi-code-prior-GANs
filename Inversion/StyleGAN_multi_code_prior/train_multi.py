...
import numpy as np
import os, sys
from networks_multi import build_discriminator, build_generator
...
for epoch in range(num_epochs):
    ...
    while n <= train_size:
      ...
      latent_codes = tf.random.normal([len(real_images), n_codes, latent_dim], 0, 1, tf.float32)
...
random_codes = tf.random.normal([batch_size, n_codes, latent_dim], 0, 1, tf.float32)